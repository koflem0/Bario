...
public class Character
{
	...
	public int id, connection, dir = 0, life = 100, maxLife = 100, mana = 100, maxMana = 100, lvl = 1, exp = 0, currentSkill = -1, skillStats = 0, attStats = 0, MF,
			RF;
	...
	public int getDamage(Monster monster, float dmgMult)
	{
		...
		if (monster.avoid > rand.nextInt(100) + atts[Network.AGI]) return 0;
		int dmast = rand.nextInt(100 - monster.getMastery()) + monster.getMastery();

		float dmg = (((wATK * (atts[0] + 100) / 100) * dmgMult));
		dmg = dmg * dmast / 100;
		dmg = (float) (dmg * (1 - monster.getDefense() / (monster.getDefense() + 22 * Math.pow(1.1, lvl))));
		if (dmg < 1) dmg = 1;
		return (int) dmg;
	}
	...
	public void loadStats()
	{
		for (PassiveSkill passive : passives)
			if (passive != null) passive.skillStats();

		for (int i = 0; i < 4; i++)
		{
			atts[i] = baseAtts[i] + inventory.getStat(i) + inventory.getStat(Network.ALLSTATS);
			for (PassiveSkill passive : passives)
				if (passive != null) atts[i] += passive.getSkillStat(i);
		}
		...
		if (classe == MAGE) maxMana = 5 * (lvl - 1);
		else maxMana = 0;
	...
	{
		if (alive) if (!invincible)
		{
			if (dmg > 0)
			{
				life -= dmg;
				if (classe == FIGHTER) if ((mana += 3 * (100 + atts[Network.SPIRIT]) / 100) > maxMana) mana = maxMana;
			}
			if (life <= 0)
			{
				life = 0;
	...
	public void update(long timePassed)
	{
		if (alive)
		{
			...
			if (classe == MAGE)
			{
				if (mana < maxMana) manaRegen += timePassed;
			...
			} else if (classe == ARCHER)
			{
				if (mana < maxMana)
				{
					...
					if ((vx == 0 && vx == 0)) manaRegen += 5 * timePassed;
				}
			...
			if (life < maxLife) lifeRegen += timePassed;
			...
			if (canMove) vx = 0.37d * xspeed;
			if (currentSkill != -1 && vy == 0) vx = 0;
			x += vx * timePassed;
			y += vy * timePassed;
			if (canMove) if (dir > 0) facingLeft = false;
			else if (dir < 0) facingLeft = true;
			if (onLadder == null) updateSkill(timePassed);
			else currentSkill = -1;
		} else
		{
	...
	public void updateSkill(long timePassed)
	{
		...
		if (skillKey == -1 || skillKey != currentSkill) if (currentSkill != -1 && !skills[currentSkill].isActive()) currentSkill = -1;

	}

	public void fall(long timePassed)
	{
		if (vy < 0.9d && onLadder == null) vy += timePassed * 0.0046d;
		if (vy > 0.9d) vy = 0.9d;
	}

	public void jump()
	{
		if (currentSkill == -1) vy = -1.45d;
	}
	...
	{
		return new Rectangle(getX() + 25, getY() + getHeight() - 15, getWidth() - 50, 20);
	}
	...
	{
		return new Rectangle(getX() + 17, getY() + 2, getWidth() - 34, getHeight() - 2);
	}
	...
	{
		return new Rectangle(getX() + getWidth() - 22, getY() + 13, 10, getHeight() - 24);
	}

	public void setClimbing(int i)
	{
		if (canMove) if (onLadder != null) vy = -i * 0.36d;
	}
